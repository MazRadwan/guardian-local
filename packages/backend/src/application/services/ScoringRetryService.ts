...
export class ScoringRetryService {
  ...
  private buildCorrectionPrompt(violations: string[]): string {
    ...
    return [
      ...
      '4. Disqualifying factors use a two-tier system:',
      '   - Hard factors (AUTOMATIC DECLINE) require recommendation = "decline"',
      '   - Remediable blockers (when no hard factors present) allow "conditional" but block "approve"',
      '   - Use exact canonical keys (underscore_separated) from the rubric',
      '',

# Story 2.3: Create File Validation Service
...
Create a dedicated file validation service that performs comprehensive validation including magic byte checking for file type verification.

---

## Prerequisites

- Complete Stories 2.1-2.2
...
```bash
cd packages/backend && pnpm add file-type
```

### File to Create

**Path:** `packages/backend/src/application/services/FileValidationService.ts`
...
- [ ] Dependencies installed (`file-type`)
- [ ] `FileValidationService` created
- [ ] Magic byte detection working
- [ ] Size validation working
- [ ] Extension validation working
- [ ] DOCX-as-ZIP detection handled (trust extension when detected as zip)
- [ ] Method renamed to `quickValidateByExtensionAndSize` (clear about what it checks)
- [ ] Warnings for mismatches (non-blocking)
...
2. Proceed to `2.4-storage-tests.md`
